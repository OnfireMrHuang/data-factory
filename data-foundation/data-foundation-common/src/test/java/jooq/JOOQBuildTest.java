...
public class JOOQBuildTest {
    ...
    public void testJOOQMysql() {
        ...
        List<SelectFieldOrAsterisk> selectFields = List.of(
                field("users.name"),
                sum(field("orders.amount").cast(INTEGER)).as("total_amount"),count(field("orders.id")).as("total_order_count")
        );

        // 子查询: union,where, 日期函数、转换函数
        // 主体sql： join、order by、limit、offset、subQuery and so on
        Query query = create.select(selectFields)
                .from(table("users"))
