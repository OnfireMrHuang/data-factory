...
use crate::components::business::collection::test_run_components::PreviewDataResponse;
...
pub fn CollectionEditPage(code: String) -> Element {
    ...
    let test_run_handler = move |_| {
        let code_clone = code_for_test.clone();
        spawn(async move {
            ...
            let task_id = match collections::execute_test_run(&code_clone).await {
                Ok(response) => {
                    logs.push(LogEntry {
                        timestamp: Local::now(),
            ...
            loop {
                ...
                // Get test run status
                match collections::get_test_run_status(&code_clone, &task_id).await {
                    Ok(status_response) => {
                        // Update steps
                        let steps: Vec<TestStep> = status_response.steps
                            ...
                            .map(|step_info| {
                                ...
                                TestStep {
                                    ...
                                    start_time: step_info.start_time.and_then(|s| {
                                        chrono::DateTime::parse_from_rfc3339(&s).ok().map(|dt| dt.with_timezone(&Local))
                                    }),
                                    end_time: step_info.end_time.and_then(|s| {
                                        chrono::DateTime::parse_from_rfc3339(&s).ok().map(|dt| dt.with_timezone(&Local))
                                    }),
                                }
                            })
                            ...
                            let new_logs: Vec<LogEntry> = logs_response.items
                                ...
                                .map(|log_item| {
                                    let level = match log_item.level {
                                        crate::models::test_run::LogLevel::Info => LogLevel::Info,
                                        crate::models::test_run::LogLevel::Success => LogLevel::Success,
                                        crate::models::test_run::LogLevel::Warning => LogLevel::Warning,
                                        crate::models::test_run::LogLevel::Error => LogLevel::Error,
                                    };
                                    let timestamp = chrono::DateTime::parse_from_rfc3339(&log_item.timestamp)
                                        .ok()
                        ...
                        if is_complete {
                            if test_success {
                                match collections::get_test_run_preview(&code_clone, &task_id).await {
                                    Ok(preview_response) => {
